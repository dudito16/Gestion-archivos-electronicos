...
import type { AnswerValue5, EditFieldsQuestion } from "../practice5.types";
...
export function EditFieldsView5({ question, disabled, onCheck }: Props) {
    ...
    <div className="space-y-md">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-sm">
        {question.fields.map((field) => (
          <div key={field.id}>
            <label htmlFor={`ef5-${question.id}-${field.id}`} className="text-label-md font-label-md font-semibold text-on-surface mb-1 block">
              {field.label}
            </label>
            <input
              id={`ef5-${question.id}-${field.id}`}
              type="text"
      ...
      <OpenTextField id={`ef5-exp-${question.id}`} label={question.explanationPrompt} value={explanation} onChange={setExplanation} rows={3} disabled={disabled} />
