...
import type { AnswerValue5, StageFlowQuestion } from "../practice5.types";
...
export function StageFlowView5({ question, disabled, onCheck }: Props) {
  ...
  return (
    <div className="space-y-md">
          ...
          return (
            ...
            >
              ...
              {FIELD_KEYS.map((key, index) => (
                <div key={key}>
                  <label htmlFor={`sf5-${question.id}-${stage.id}-${key}`} className="text-caption font-caption text-on-surface-variant mb-0.5 block sm:hidden">
                    {question.fieldLabels[index]}
                  </label>
                  <textarea
                    id={`sf5-${question.id}-${stage.id}-${key}`}
                    rows={2}
                    value={row[key] ?? ""}
                    disabled={disabled}
      ...
      <div>
        <label htmlFor={`sf5-critical-${question.id}`} className="text-label-md font-label-md font-semibold text-on-surface mb-1 block">
          Etapa crítica
        </label>
        <select
          id={`sf5-critical-${question.id}`}
