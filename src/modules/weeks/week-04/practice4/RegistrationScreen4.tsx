...
import { TOTAL_QUESTIONS_4 } from "./practice4.data";

interface Props {
  onStart: (nombreCompleto: string, nombreGrupo: string) => void;
}

export function RegistrationScreen4({ onStart }: Props) {
  ...
  return (
    <div className="rounded-xl border border-outline-variant bg-surface-container-lowest p-lg space-y-md max-w-xl mx-auto text-center">
      <div>
        <span className="text-label-md font-label-md font-bold uppercase tracking-wider text-primary-container">Práctica calificada · Semana 4</span>
        <h3 className="text-headline-md font-headline-md text-on-surface mt-1">Práctica Calificada 4 — Simulador de decisiones sobre metadatos</h3>
        <p className="text-caption font-caption text-on-surface-variant mt-1">{TOTAL_QUESTIONS_4} actividades · 20 puntos</p>
      </div>

      <div className="space-y-sm text-left">
        <div>
          <label htmlFor="pc4-nombre" className="text-label-md font-label-md font-semibold text-on-surface mb-1 block">
        ...
        <div>
          <label htmlFor="pc4-grupo" className="text-label-md font-label-md font-semibold text-on-surface mb-1 block">
