...
import type { AnswerValue4, EditFieldsQuestion } from "../practice4.types";
...
export function EditFieldsView({ question, disabled, onCheck }: Props) {
    ...
    <div className="space-y-md">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-sm">
        {question.fields.map((field) => (
          <div key={field.id}>
            <label htmlFor={`ef-${question.id}-${field.id}`} className="text-label-md font-label-md font-semibold text-on-surface mb-1 block">
              {field.label}
            </label>
            <input
              id={`ef-${question.id}-${field.id}`}
              type="text"
      ...
      <OpenTextField id={`ef-exp-${question.id}`} label={question.explanationPrompt} value={explanation} onChange={setExplanation} rows={3} disabled={disabled} />
