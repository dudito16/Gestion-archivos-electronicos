...
/**
 * Consistent "lab card" chrome for every Week 6 activity or workshop exercise. Unlike Week 4/5's
 * `ActivityShell` (which reads completion from a single context), this one takes `done` as a prop
 * so the same shell can serve both Week 6 pools — "Actividades" and "Taller" — which track
 * progress in two independent contexts/storage keys.
 */
export function ActivityShell({ id, number, title, objective, instructions, done, children }: ActivityShellProps) {
  return (
    <div
      id={id}
      className={cn(
