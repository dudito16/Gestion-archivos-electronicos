...
import type { AnswerValue6, MatrixBuilderQuestion } from "../practice6.types";
...
export function MatrixBuilderView6({ question, disabled, onCheck }: Props) {
  const idBase = useId();
  ...
  return (
    <div className="space-y-md">
      {question.chipSuggestions && question.chipSuggestions.length > 0 && (
        <div>
          <p className="text-caption font-caption text-on-surface-variant mb-1">Elementos mencionados en el caso (toca uno para usarlo en una fila vacía):</p>
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-xs">
                {question.columns.map((col) => (
                  <div key={col.key}>
                    <label htmlFor={`mb6-${row.id}-${col.key}`} className="text-caption font-caption text-on-surface-variant mb-0.5 block">
                      {col.label}
                    </label>
                    {col.type === "select" ? (
                      <select
                        id={`mb6-${row.id}-${col.key}`}
